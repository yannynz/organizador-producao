...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, AdvancedFiltersComponent, OrderDetailsModalComponent],
...
export class DeliveredListComponent implements OnInit, OnDestroy {
  ...
  statusDescriptions: { [key: number]: string } = {
    0: 'Em Produção', 1: 'Cortada', 7: 'Montada',
    2: 'Pronto para Entrega', 3: 'Saiu para Entrega',
    4: 'Retirada', 5: 'Entregue', 6: 'Tirada',
  };
  ...
  baseFilters: OrderFilters = { status: [4, 5], sort: 'dataEntrega,desc,id,desc' };
    ...
    this.subs.add(
      this.websocket.watchOrders().subscribe(msg => {
        ...
        if (typeof msg?.body === 'string') text = msg.body;
          ...
          try { text = new TextDecoder('utf-8').decode(msg.body); } catch { text = String(msg.body); }
        }
        ...
        const passes = (o.status === 4 || o.status === 5) &&
        ...
        if (this.localMode) {
          ...
          this.localAll = (idx > -1) ? this.localAll.map(x => x.id === o.id ? o : x)
            : (passes ? [o, ...this.localAll] : this.localAll);
          if (!passes) this.localAll = this.localAll.filter(x => x.id !== o.id);
  ...
  onApplyFilters(ev: { search: string; filters: OrderFilters }) {
    this.currentFilters = { ...(ev.filters || {}), status: [4, 5], sort: 'dataEntrega,desc,id,desc' };
    this.reload();
  }
  onClearFilters() {
  ...
  openDetails(o: orders) { this.selectedOrder = o; this.detailsOpen = true; }
  closeDetails() { this.selectedOrder = null; this.detailsOpen = false; }
  ...
  gotoPage(p: number) {
    ...
    if (this.localMode) {
      // client-side
      const maxPages = Math.max(1, Math.ceil(this.localAll.length / this.pageSize));
    ...
    this.loading = true;
    this.orderService.searchDeliveredCursor({
      limit: this.pageSize,
      cursor: startCursor,
      strategy: 'DATE_ID',
      filters: this.currentFilters
    }).subscribe((res: CursorPage<orders>) => {
      this.localMode = false;

      this.pageCache[p] = res.items;
      this.pageMeta[p] = { nextCursor: res.nextCursor, hasMore: res.hasMore };
  ...
  gotoPrev() { if (this.currentPage > 1) this.gotoPage(this.currentPage - 1); }
  gotoNext() {
    if (this.localMode) {
      const maxPages = Math.max(1, Math.ceil(this.localAll.length / this.pageSize));
  ...
  private fetchLocalAndGo(p: number) {
    this.orderService.getOrders().subscribe(all => {
      const filtered = this.sortRows(this.applyFiltersLocal(all, this.currentFilters));
      this.localMode = true;
    ...
    return list.filter(o => {
      ...
      if (q) {
        ...
        const hay =
          ...
          S(o.veiculo).includes(q) || S(o.recebedor).includes(q) || S(o.montador).includes(q) ||
          S(o.observacao).includes(q) || D(o.dataH).includes(q) || D(o.dataEntrega).includes(q);
        if (!hay) return false;
      }
      return true;
    });
