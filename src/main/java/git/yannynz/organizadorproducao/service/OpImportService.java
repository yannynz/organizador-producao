...
private void notifyOpImported(OpImport saved, OpImportRequestDTO req, boolean linkedNow) {
  ...
  public void importar(OpImportRequestDTO req) {
    ...
    if (req.getDataRequeridaEntrega() != null && !req.getDataRequeridaEntrega().isBlank()) {
        try {
            ...
            log.error("Falha ao parsear dataRequeridaEntrega '{}': {}", req.getDataRequeridaEntrega(), e.getMessage());
        }
    }
    ...
    Boolean newPapelCalibrado =
        requestedPapelCalibrado != null ? requestedPapelCalibrado : previousPapelCalibrado;
    ...
    if (maybeOrder.isPresent()) {
        ...
        if (orderUpdated) { orderRepo.save(order); notifyOrder(order); }
    }
    ...
    if (savedOp.getDataRequeridaEntrega() == null) {
        orderRepo.findTopByNrOrderByIdDesc(numeroOp).ifPresent(ord -> {
            ...
            if (pr != null) {
                ...
                java.time.ZonedDateTime base = java.time.ZonedDateTime.now(tz).withHour(18).withMinute(0).withSecond(0).withNano(0);
                java.time.ZonedDateTime dt = null;
                switch (pr) {
                    case "VERMELHO": dt = base; break;                 // hoje 18:00
                    case "AMARELO": dt = base.plusDays(1); break;      // amanhã 18:00
                    case "AZUL": dt = base.plusDays(2); break;         // depois de amanhã 18:00
                    default: break;                                      // VERDE: sem prazo
                }
                if (dt != null) {
                    ...
                    if (!obsLower2.contains("entrega=")) appendTag(new StringBuilder(obs), "entrega=" + dt);
                    orderRepo.save(ord);
                    notifyOrder(ord);
                }
            }
  ...
  private ZonedDateTime parseToZoned(String s, ZoneId defaultZone) {
    ...
    // ISO com offset e/ou região (ex.: 2025-09-16T00:00:00-03:00[America/Sao_Paulo])
...
private boolean containsTag(String obs, String chaveLower) {
  ...
  // RabbitMQ
  // ---------------------------
  // Listener definido em AmqpConfig (OpImportedListener) já chama service.importar(req)
    ...
    repo.findByNumeroOp(nr).ifPresent(op -> {
      ...
      orderRepo.findById(facaId).ifPresent(order -> {
        ...
        if (!alreadyLinked && op.isEmborrachada() && !order.isEmborrachada()) {
          order.setEmborrachada(true);
          orderUpdated = true;
          log.info("[LINK] Pedido {} recebeu emborrachada=true da OP {} (link tardio)", order.getId(), op.getNumeroOp());
        } else {
          if (order.isEmborrachada()) {
            if (!op.isEmborrachada()) {
              op.setEmborrachada(true);
              opUpdated = true;
              log.info("[LINK] OP {} sincronizada com pedido {} emborrachada=true", op.getNumeroOp(), order.getId());
            }
            if (manualLockEmborrachada) {
              op.setManualLockEmborrachada(false);
              opUpdated = true;
              log.info("[LINK] OP {} removeu lock manual emborrachada ao detectar pedido {}=true", op.getNumeroOp(), order.getId());
            }
          } else if (op.isEmborrachada() && !manualLockEmborrachada) {
            order.setEmborrachada(true);
            orderUpdated = true;
            log.info("[LINK] Pedido {} ajustado para emborrachada=true a partir da OP {}", order.getId(), op.getNumeroOp());
          }
        }
        ...
        if (!isBlank(order.getModalidadeEntrega())) {
          if (isBlank(op.getModalidadeEntrega()) || !Objects.equals(order.getModalidadeEntrega(), op.getModalidadeEntrega())) {
        ...
        if (!isBlank(order.getUsuarioImportacao())) {
          if (isBlank(op.getUsuarioImportacao()) || !Objects.equals(order.getUsuarioImportacao(), op.getUsuarioImportacao())) {
        ...
        if (order.isPertinax()) {
          ...
          if (lockPertinax) {
            op.setManualLockPertinax(false);
            opUpdated = true;
            log.info("[LINK] OP {} removeu lock pertinax após pedido {}=true", op.getNumeroOp(), order.getId());
          }
        } else {
          if (Boolean.TRUE.equals(opPertinax) && !lockPertinax) {
            ...
            op.setManualLockPertinax(false);
            opUpdated = true;
            log.info("[LINK] OP {} limpou lock pertinax pois pedido {} segue false", op.getNumeroOp(), order.getId());
          }
        }
        ...
        if (order.isPoliester()) {
          ...
          if (lockPoliester) {
            op.setManualLockPoliester(false);
            opUpdated = true;
            log.info("[LINK] OP {} removeu lock poliester após pedido {}=true", op.getNumeroOp(), order.getId());
          }
        } else {
          if (Boolean.TRUE.equals(opPoliester) && !lockPoliester) {
            ...
            op.setManualLockPoliester(false);
            opUpdated = true;
            log.info("[LINK] OP {} limpou lock poliester pois pedido {} segue false", op.getNumeroOp(), order.getId());
          }
        }
        ...
        if (order.isPapelCalibrado()) {
          if (!Boolean.TRUE.equals(opPapel)) {
            ...
            opUpdated = true;
            log.info("[LINK] OP {} recebeu papelCalibrado=true do pedido {}", op.getNumeroOp(), order.getId());
          }
          if (lockPapel) {
            op.setManualLockPapelCalibrado(false);
            opUpdated = true;
            log.info("[LINK] OP {} removeu lock papelCalibrado após pedido {}=true", op.getNumeroOp(), order.getId());
          }
        } else {
          if (Boolean.TRUE.equals(opPapel) && !lockPapel) {
            order.setPapelCalibrado(true);
            orderUpdated = true;
            log.info("[LINK] Pedido {} recebeu papelCalibrado=true da OP {}", order.getId(), op.getNumeroOp());
          } else if (lockPapel && !Boolean.TRUE.equals(opPapel)) {
            op.setManualLockPapelCalibrado(false);
            opUpdated = true;
            log.info("[LINK] OP {} limpou lock papelCalibrado pois pedido {} segue false", op.getNumeroOp(), order.getId());
          }
        }
      ...
      if (previousEmborrachada != order.isEmborrachada()) {
        if (!order.isEmborrachada() && op.isEmborrachada()) {
          if (!op.isManualLockEmborrachada()) {
            ...
            dirty = true;
            log.info("[ORDER] Pedido {} marcou lock emborrachada=false na OP {}", order.getId(), op.getNumeroOp());
          }
      ...
      if (previousPertinax != order.isPertinax()) {
        if (!order.isPertinax() && Boolean.TRUE.equals(op.getPertinax())) {
          if (!op.isManualLockPertinax()) {
            ...
            dirty = true;
            log.info("[ORDER] Pedido {} marcou lock pertinax=false na OP {}", order.getId(), op.getNumeroOp());
          }
      ...
      if (previousPoliester != order.isPoliester()) {
        if (!order.isPoliester() && Boolean.TRUE.equals(op.getPoliester())) {
          if (!op.isManualLockPoliester()) {
            ...
            dirty = true;
            log.info("[ORDER] Pedido {} marcou lock poliester=false na OP {}", order.getId(), op.getNumeroOp());
          }
      ...
      if (previousPapelCalibrado != order.isPapelCalibrado()) {
        if (!order.isPapelCalibrado() && Boolean.TRUE.equals(op.getPapelCalibrado())) {
          if (!op.isManualLockPapelCalibrado()) {
            ...
            dirty = true;
            log.info("[ORDER] Pedido {} marcou lock papelCalibrado=false na OP {}", order.getId(), op.getNumeroOp());
          }
    ...
    repo.findAll().stream()
        ...
        .forEach(o -> orderRepo.findTopByNrOrderByIdDesc(o.getNumeroOp()).ifPresent(f -> {
          ...
          if (!linked && o.isEmborrachada() && !f.isEmborrachada()) {
            f.setEmborrachada(true);
            orderUpdated = true;
            log.info("[RECONCILE] Pedido {} sincronizado para emborrachada=true via OP {}", f.getId(), o.getNumeroOp());
          } else {
            if (f.isEmborrachada()) {
              if (!o.isEmborrachada()) {
                o.setEmborrachada(true);
                opUpdated = true;
                log.info("[RECONCILE] OP {} recebeu emborrachada=true do pedido {}", o.getNumeroOp(), f.getId());
              }
              if (lockEmborrachada) {
                o.setManualLockEmborrachada(false);
                opUpdated = true;
                log.info("[RECONCILE] OP {} removeu lock emborrachada após pedido {}=true", o.getNumeroOp(), f.getId());
              }
            } else if (o.isEmborrachada() && !lockEmborrachada) {
              f.setEmborrachada(true);
              orderUpdated = true;
              log.info("[RECONCILE] Pedido {} recebeu emborrachada=true da OP {}", f.getId(), o.getNumeroOp());
            }
          }
          ...
          if (!isBlank(f.getModalidadeEntrega())) {
            if (isBlank(o.getModalidadeEntrega()) || !Objects.equals(f.getModalidadeEntrega(), o.getModalidadeEntrega())) {
          ...
          if (!isBlank(f.getUsuarioImportacao())) {
            if (isBlank(o.getUsuarioImportacao()) || !Objects.equals(f.getUsuarioImportacao(), o.getUsuarioImportacao())) {
          ...
          if (f.isPertinax()) {
            if (!Boolean.TRUE.equals(o.getPertinax())) {
              o.setPertinax(true);
              opUpdated = true;
              log.info("[RECONCILE] OP {} recebeu pertinax=true do pedido {}", o.getNumeroOp(), f.getId());
            }
            if (lockPertinax) {
              o.setManualLockPertinax(false);
              opUpdated = true;
              log.info("[RECONCILE] OP {} removeu lock pertinax após pedido {}=true", o.getNumeroOp(), f.getId());
            }
          } else {
            if (Boolean.TRUE.equals(o.getPertinax()) && !lockPertinax) {
              f.setPertinax(true);
              orderUpdated = true;
              log.info("[RECONCILE] Pedido {} recebeu pertinax=true da OP {}", f.getId(), o.getNumeroOp());
            } else if (lockPertinax && !Boolean.TRUE.equals(o.getPertinax())) {
              o.setManualLockPertinax(false);
              opUpdated = true;
              log.info("[RECONCILE] OP {} limpou lock pertinax pois pedido {} permanece false", o.getNumeroOp(), f.getId());
            }
          }
          ...
          if (f.isPoliester()) {
            if (!Boolean.TRUE.equals(o.getPoliester())) {
              o.setPoliester(true);
              opUpdated = true;
              log.info("[RECONCILE] OP {} recebeu poliester=true do pedido {}", o.getNumeroOp(), f.getId());
            }
            if (lockPoliester) {
              o.setManualLockPoliester(false);
              opUpdated = true;
              log.info("[RECONCILE] OP {} removeu lock poliester após pedido {}=true", o.getNumeroOp(), f.getId());
            }
          } else {
            if (Boolean.TRUE.equals(o.getPoliester()) && !lockPoliester) {
              f.setPoliester(true);
              orderUpdated = true;
              log.info("[RECONCILE] Pedido {} recebeu poliester=true da OP {}", f.getId(), o.getNumeroOp());
            } else if (lockPoliester && !Boolean.TRUE.equals(o.getPoliester())) {
              o.setManualLockPoliester(false);
              opUpdated = true;
              log.info("[RECONCILE] OP {} limpou lock poliester pois pedido {} permanece false", o.getNumeroOp(), f.getId());
            }
          }
          ...
          if (f.isPapelCalibrado()) {
            if (!Boolean.TRUE.equals(o.getPapelCalibrado())) {
              ...
              opUpdated = true;
              log.info("[RECONCILE] OP {} recebeu papelCalibrado=true do pedido {}", o.getNumeroOp(), f.getId());
            }
            if (lockPapel) {
              o.setManualLockPapelCalibrado(false);
              opUpdated = true;
              log.info("[RECONCILE] OP {} removeu lock papelCalibrado após pedido {}=true", o.getNumeroOp(), f.getId());
            }
          } else {
            if (Boolean.TRUE.equals(o.getPapelCalibrado()) && !lockPapel) {
              f.setPapelCalibrado(true);
              orderUpdated = true;
              log.info("[RECONCILE] Pedido {} recebeu papelCalibrado=true da OP {}", f.getId(), o.getNumeroOp());
